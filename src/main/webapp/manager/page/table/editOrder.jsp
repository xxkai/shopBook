    	...
    	<input type="hidden" name="orderId" value="${order.orderId}"/>
          ...
          <div class="layui-form-item">
            	<label class="layui-form-label required">送货地址</label>
          		<div class="layui-input-inline">
               		<input type="text"  name="orderAddr" value="${order.orderAddr}" lay-verify="required" placeholder="请输入详系地址"  autocomplete="off" class="layui-input">
                </div>
          </div>
            <div class="layui-form-item">
            	<label class="layui-form-label required">收件人姓名</label>
            	<div class="layui-input-block">
                <input type="text" name="orderName" value="${order.orderName}" required lay-verify="required" lay-reqtext="收货人不能为空" placeholder="请输入收货人" value="" class="layui-input">
                <tip>填写收货人姓名。</tip>
            </div>
            </div>
            <div class="layui-form-item">
            	...
            	<div class="layui-input-block">
                	<input type="number" name="orderNum" value="${order.orderNum}" required lay-verify="required"  value="" class="layui-input">
           		</div>
        	</div>
        <div class="layui-form-item" >
            <div class="layui-input-block">
...
<script>
    layui.use(['form', 'table','layedit','laydate'], function () {       
    	var form = layui.form;
          ...
          var $ = layui.jquery;
        $.ajax({
     	   url: '${ctx}/manager/BooksAction?method=books',
     	   dataType: 'json',
     	   type: 'get',
     	   success: function (data) {
     	   	$.each(data, function (index, item) {
     	   		$('#bookId').append(new Option(item.title,item.bookId));// 下拉菜单里添加元素
     	   		$('#bookId').val('${order.bookId}');
			});
     	   	layui.form.render("select");
     	   }
        });
        $.ajax({
      	   url: '${ctx}/manager/UserAction?method=users',
      	   dataType: 'json',
      	   type: 'get',
      	   success: function (data) {
      	   	$.each(data, function (index, item) {
        ...
        form.on('submit(saveBtn)', function (data) {
            ...
            var index = layer.alert(JSON.stringify(data.field), {
            	...
            	$.ajax({
          		  type:'post',
          		  url:'${ctx}/manager/OrderAction?method=insert',
          		  data:data.field,
          		  dataType:'json',
