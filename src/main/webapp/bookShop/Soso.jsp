...
<body>
	...
	<div class="idx-container">
		<div class="container" style="height:0px;">
			...
			<div style="width: 100%; font-size: 25px;margin: 30px 0">${category.categoryName }界面</div>
				...
				<div style="width: 100%;height: 50px;float: left;text-align: center;">
					<form id="pageForm" action="ShoppingUserAction?method=soSo&categoryId=${category.categoryId}" method="post">
					<table>
						<tr>
							<td align="center" style="font-size: 17px">
								共${num}条记录
								<a href="ShoppingUserAction?method=soSo&categoryId=${category.categoryId}&&page=${page}">首页</a>
								<c:if test="${page eq 1}" >
									<span style="color:#808A87">上一页</span>
								</c:if>
								<c:if test="${page ne 1}" >
									<a href="ShoppingUserAction?method=soSo&categoryId=${category.categoryId}&&page=${page-1}">上一页</a>
								</c:if>
								...
								<c:if test="${page ne allPage+1}" >
									<a href="ShoppingUserAction?method=soSo&categoryId=${category.categoryId}&&page=${page+1}">下一页</a>
								</c:if>
								<a href="ShoppingUserAction?method=soSo&categoryId=${category.categoryId}&&page=${allPage+1}">尾页</a>	
								每页${num}条
